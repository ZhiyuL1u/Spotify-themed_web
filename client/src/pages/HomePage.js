...
export default function HomePage() {
  ...
  useEffect(() => {
    ...
    fetch(`https://${config.server_host}:${config.server_port}/author/name`)
        ...
        .then(resJson => {
          console.log(resJson)
          // Assuming the API returns an object with a name field for the author's name
  ...
  const songColumns = [
    ...
    },
    {
    ...
    },
      {
          field: 'plays',
          headerName: 'Plays',
          renderCell: (row) => <a>{row.plays}</a>
      },
  ];
    const albumColumns = [

        {
            field: 'title', // The field from the album data representing the album's title
          headerName: 'Album Title',
          // Use renderCell to customize how the cell displays the title as a clickable link
          renderCell: (row) => <NavLink to={`/albums/${row.album_id}`}>{row.title}</NavLink>
      },
      {
          field: 'plays', // The field from the album data representing the number of plays
          headerName: 'Plays',
          renderCell: (row) => <a>{row.plays}</a>
      },
  ];
  ...
  // Hint: recall the schema for an album is different from that of a song (see the API docs for /top_albums). How does that impact the "field" parameter and the "renderCell" function for the album title column?
    ...
    return (
        <Container>
            ...
            <Divider/>
            <h2>Top Songs</h2>
            <LazyTable route={`https://${config.server_host}:${config.server_port}/top_songs`} columns={songColumns}/>
            <Divider/>
